      ...
      class CGroups
        ...
        class Libcgroup
          ...
          def initialize(uuid)
            ...
            @cgroup_root = @config.get("OPENSHIFT_CGROUP_ROOT", @@DEFAULT_CGROUP_ROOT)
            @cgroup_path = "#{@cgroup_root}/#{@uuid}"

            if not @@SUBSYSTEMS
              @@SUBSYSTEMS = @config.get("OPENSHIFT_CGROUP_SUBSYSTEMS", @@DEFAULT_CGROUP_SUBSYSTEMS).strip.split(',')
            end
            @subsystems = @@SUBSYSTEMS

            if not @@PARAMETERS
              subsys = @subsystems_cache.map { |subsys| "-g #{subsys}" }.join(' ')
              out, err, rc = ::OpenShift::Runtime::Utils::oo_spawn("cgget #{subsys} /")
              if rc != 0
                raise RuntimeError, "Could not determine Cgroup parameters"
              end
              @@PARAMETERS = parse_cgget(out).keys.sort
            end
          ...
          def create(defaults={})
            ...
            to_store = Hash.new {|h,k| h[k]={}}
            to_store["net_cls.classid"] = newcfg["net_cls.classid"]
          ...
          def classify_processes
            ...
            while not pids.empty?
              ...
              out, err, rc = ::OpenShift::Runtime::Utils::oo_spawn("cgclassify -g #{subsys}:#{@cgroups_path} #{pidout}")
            end
          ...
          def processes
            ...
            out, err, rc = ::OpenShift::Runtime::Utils::oo_spawn("ps -o pid,cgroup --no-headers")
            out.each_line do |proc|
              pid, cgroup = proc.strip.split
          ...
          def cgcreate
            out, err, rc = ::OpenShift::Runtime::Utils::oo_spawn("cgcreate -t #{@uuid}:#{@uuid} -g #{@subsystems}:#{@cgroup_path}")
            case rc
            when 0
              return nil
            else
          ...
          def cgdelete
            out, err, rc = ::OpenShift::Runtime::Utils::oo_spawn("cgdelete #{@subsystems}:#{@cgroup_path}")
            nil
          end
          ...
          def update_cgrules(recreate=true)
            overwrite_with_safe_swap(@@CGRULES) do |f_in, f_out|
              ...
              if recreate
                f_out.puts("#{@uuid}\t#{@subsystems}\t#{@cgroup_path}")
              end
            end
